...
# st.write("현재 적용된 폰트:", plt.rcParams['font.family'])
...
# matplotlib Figure 생성 함수 (원래 update() 함수의 내용을 포함)
def create_figure():
    fig, ax = plt.subplots()
    ...
    for seat, (x, y) in seat_positions.items():
        ...
        # 3등분 셀 (왼쪽: PM2.5, 중앙: CO2, 오른쪽: occupancy)
        cell_w = rect_width / 3
        left_x = lower_left[0]
        mid_x = lower_left[0] + cell_w
        right_x = lower_left[0] + 2 * cell_w

        # API 데이터 호출 (성혁(seat12)는 API 없음)
        if seat in api_urls:
...
# Streamlit의 빈 컨테이너 생성
placeholder = st.empty()

# st_autorefresh를 사용한 자동 새로 고침
